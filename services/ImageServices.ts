...
import { UploadErrorResult, UploadSuccessResult } from "../types/types";
...
export const uploadFileToCloudinary = async (
  ...
  try {
    ...
    console.log(
      `[Service] Завантаження файлу ${file.name} на Cloudinary до папки ${folder}...`
    );
    ...
    console.log(
      `[Service] Успішно завантажено: ${file.name} -> ${result.secure_url}`
    );
    return {
      success: true,
    ...
    const errorMessage =
      ...
      (error instanceof Error
        ? error.message
        : "Невідома помилка під час завантаження");
    console.error(
      `[Service] Помилка завантаження файлу ${file.name}:`,
      errorMessage
    );
    return {
      success: false,
